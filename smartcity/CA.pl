...
my $RET = 0;
my $WHAT = shift @ARGV || "";
my @OPENSSL_CMDS = ("req", "ca", "pkcs12", "x509", "verify", "ec");
...
if ($WHAT eq '-newcert' ) {
    ...
    $RET = run("$ECPARAM -genkey -name secp521r1 -out  $NEWKEY");
    

    $RET = run("$REQ -new $1 -key $NEWKEY -out $NEWREQ $DAYS $EXTRA{req}");
    #RET = run("$REQ -new $1 -key $2 -out $NEWREQ $DAYS $EXTRA{req}");
    ...
    if ($FILE ne "") {
		...
		print "Generate new ecc key\n";
		#openssl ecparam -genkey -name prime256v1 -out ecprivkey.pem
		#secp521r1
		
		$RET = run("$ECPARAM -genkey -name secp521r1 -out  ${CATOP}/private/$CAKEY");
		
		$RET = run("cat ${CATOP}/private/$CAKEY");
		
		
        #$RET = run("$REQ -new -keyout  ${CATOP}/private/$CAKEY -out ${CATOP}/$CAREQ $EXTRA{req}");
		$RET = run("$REQ -new -key  ${CATOP}/private/$CAKEY -out ${CATOP}/$CAREQ $EXTRA{req}");
