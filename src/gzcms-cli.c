...
void help_print_cms() {
    printf("\n"
      ...
      ANSI_COLOR_CYAN_BOLD"    --encrypt ...        "ANSI_COLOR_RESET"\n"
                          "                         Encrypt input file for given recipient certificates.\n"
                          "                         output file is the enveloped with CMS(PEM) type.\n"
      ANSI_COLOR_CYAN_BOLD"    --decrypt ...        "ANSI_COLOR_RESET"Decrypt enveloped message\n"
...
void help_print_keypass() {
    ...
    "\n"
      ...
      ANSI_COLOR_CYAN_BOLD"    --keypass {init|list|list2|add||rename|revoke|delete|truncate|password}"ANSI_COLOR_RESET"\n"
                          ...
                          "                         revoke   mark valid passphrase status as 'revoked'\n"                          
...
int verify_master_password(char *keydb_file, int verify_digest ) 
{
    ...
    //if( verify_digest == 1 && !is_file_exists(keydb_file)) {
...
//--------------------------------------------------------------------------------                
    ...
    if(operation == SMIME_ENCRYPT || operation == SMIME_DECRYPT) {
        ...
        if(encrypt_type || operation == SMIME_ENCRYPT) {
            //D_printf("operation: %d, encrypt_type=%s\n", operation, encrypt_type);
            //make_uppercase(encrypt_type);
            //D_printf("encrypt_type=%s\n", encrypt_type);
            if(!strcmp(encrypt_type, "ECCP2") || !strcmp(decrypt_type, "eccp2")) {
                D_printf("operation(SMIME_ENCRYPT:%d) --> [ECCP2_ENCRYPT]\n", SMIME_ENCRYPT);
                operation = ECCP2_ENCRYPT;
                D_printf("operation changed: %d\n", operation);
            }
        }
        else if(decrypt_type || operation == SMIME_DECRYPT) {
            D_printf("operation: %d, decrypt_type=%s\n", operation, decrypt_type);
            //make_uppercase(decrypt_type);
            D_printf("encrypt_type=%s\n", decrypt_type);
            if(!strcmp(decrypt_type, "ECCP2") || !strcmp(decrypt_type, "eccp2")) {
                D_printf("operation(SMIME_DECRYPT:%d) --> [ECCP2_DECRYPT]\n", SMIME_DECRYPT);
                operation = ECCP2_DECRYPT;
                D_printf("operation changed: %d\n", operation);
            }
        }
    ...
    //envelopedData 복호화
        ...
        return gzpki_cms_decrypt_file(config_args, infile, outfile, key_file, passin ,cms_opt);
    }
    //SignedData 생성
