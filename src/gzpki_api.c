    ...
    int opt) 
{
    ...
    if(operation == SMIME_ENCRYPT || operation == SMIME_DECRYPT || operation == SMIME_SIGN || operation == SMIME_VERIFY) 
    {
        ...
        if(intype == FORMAT_FILE) {
            ...
            IF_VERBOSE printf("INPUT_DATA: %s\n", inputdata);
            IF_VERBOSE printf("INPUT_DATA: %s\n", bin2hex(inputdata, indata_len));
            ...
            if(0 == (opt % ECCP2_BASE64_IN)) {
                IF_VERBOSE printf("eccp2 base64 in mode with ecc p2 encrypt, indatalen=%d\n", indata_len);
                ...
                IF_VERBOSE printf("base64 decode, indata=[%s],  decode length=[%d]\n", bin2hex(indata, indata_len), decode_len);
                
            }
            else { 
                IF_VERBOSE printf("eccp2, intype=FORMAT_FILE with ecc p2 encrypt...\n");
                indata = inputdata;
            ...
            if(0 == (opt % ECCP2_BASE64_IN)) {
                IF_VERBOSE printf("base64(buffer) in mode with ecc p2 encrypt...\n");
                int decode_len = inbuffer_len / 4 * 3;

                indata = (unsigned char *)decode64(inbuffer, inbuffer_len);
                indata_len = decode_len;
                IF_VERBOSE printf("base64(buffer) indata=[%s],  decode length=[%d]\n", indata, decode_len);
        ...
        if(operation == ECCP2_ENCRYPT) {
            ...
            //SECRET 값을 별도 파일(인증서파일명.eck)에서 읽는다. 
                ...
                D_printf("K from secret file: %s\n  K: %s\n", secretfile_k, ecpk);
            }
            else if(keyfile) {
                sprintf(secretfile_k, "%s", keyfile);
                r = gzpki_eccp2_read_secret(secretfile_k, ecpk, &ecpk_len, 0);
                D_printf("K from key/param file: %s\n  K: %s\n", secretfile_k, ecpk);
            }
            else {
                r = gzpki_eccp2_read_secret_from_certfile(L_certfile, ECCP2_HEADER_K, ecpk, &ecpk_len, 0);
                D_printf("default: K from certfile: %s\n K:\n", ecpk); 
            }
            ...
            if(r == 0 ) {
                if(outtype == FORMAT_FILE) {
                    ...
                    *outbuffer_len = out_len;
                    if(0==(opt % ECCP2_BASE64_OUT)) {
                        IF_VERBOSE fprintf(stderr, "FORMAT_MEM/base64 out option\n");
                        int encode_len = 4 * ((out_len + 2) / 3);
                        unsigned char *outdata = (unsigned char *)base64(out, out_len);
        ...
        //END of ECCP2 Encryption
            ...
            if(r == 0) {
                printf("success: eccp2 deryption\n");
            ...
            /*if(r == 0 ) {
                if(outtype == FORMAT_FILE) {
                    ...
                    printf("success:ecc p2 decryption: file=%s\n", outfile);
                }
                else if(outtype == FORMAT_MEM) {
                     out[out_len] = 0;
                    *outbuffer_len = out_len;
                     *outbuffer = (char *)malloc(out_len);
                    //snprintf(outbuffer[0], out_len, "%s", out);
                    memcpy(outbuffer[0], out, out_len);
                    
                }
                else {
                    printf("success:invalid out type\n", outfile);
                }
            }
            else 
