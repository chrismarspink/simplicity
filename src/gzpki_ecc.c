...
int CMM_P1_encrypt(const EC_KEY *key, const EVP_MD *digest, unsigned char  *msg, int msg_len, unsigned char *ciphertext_buf, int *ciphertext_len)
{
    ...
    printf("** debug ENC PUBLIC KEY : %s\n", EC_POINT_point2hex(group, P, POINT_CONVERSION_UNCOMPRESSED, ctx));
    ...
    printf("** debug: BIGNUM k generated: %s\n", BN_bn2hex(k));
    ...
    //point_conversion_form_t form = POINT_CONVERSION_UNCOMPRESSED;
    printf("** debug: k =gen=> ECPOINT kG: %s\n", EC_POINT_point2hex(group, kG, POINT_CONVERSION_UNCOMPRESSED, ctx));
    ...
    printf("** debug: EC_POINT ==> BN x1: %s\n", BN_bn2hex(x1));
    printf("** debug: EC_POINT ==> BN y1: %s\n", BN_bn2hex(y1));
    ...
    printf("** debug: k * PUBKEY =gen=> ECPOINT kP: %s\n", EC_POINT_point2hex(group, kP, POINT_CONVERSION_UNCOMPRESSED, ctx));

    if (!EC_POINT_get_affine_coordinates(group, kP, x2, y2, ctx)) {
        //SM2err(SM2_F_SM2_ENCRYPT, ERR_R_EC_LIB);
        goto done;
    }
    printf("** debug: EC_POINT(kP) ==> BN x2: %s\n", BN_bn2hex(x2));
    printf("** debug: EC_POINT(kP) ==> BN y2: %s\n", BN_bn2hex(y2));
    ...
    printf("** debug msg_len: %d\n", msg_len); 
    printf("** debug msg_mask:\n"); 
    for (i = 0; i != msg_len; ++i)
        printf("%d: ", msg_mask[i]);

    printf("** debug ---------- \n"); 

    printf("** debug enrypted msg \n"); 
                ...
                unsigned long ciphertext_len, unsigned char *ptext_buf, int *ptext_len)
{
    ...
    //TEST
    BIGNUM *x1tmp = NULL,  *y1tmp = NULL;

    //TEST
    BIGNUM *x2tmp = NULL,  *y2tmp = NULL;
    ...
    x2 = BN_CTX_get(ctx);
    y2 = BN_CTX_get(ctx);
    x1tmp = BN_CTX_get(ctx);
    y1tmp = BN_CTX_get(ctx);
    x2tmp = BN_CTX_get(ctx);
    y2tmp = BN_CTX_get(ctx);
    ...
    //Test
    C1Tmp = EC_POINT_new(group);
    if (C1Tmp == NULL) {
        printf("error:CMM_P1_decrypt: C1Tmp: ECRYPT, MALLOC_FAILURE)\n");
        goto done;
    }

    printf("** DECRYPT EC P1 \n"); 
    printf("** debug: C1x: %s\n", BN_bn2hex(sm2_ctext->C1x));
    printf("** debug: C1y: %s\n", BN_bn2hex(sm2_ctext->C1y));
    ...
    ///TEST
    if (!EC_POINT_set_affine_coordinates(group, C1Tmp, sm2_ctext->C1x, sm2_ctext->C1y, ctx)){
        printf("error:CMM_P1_decrypt: DECRYPT, C1Tmp, ERR_R_EC_LIB\n");
        goto done;
    }
    
    printf("** debug: C1 from C1x, C1y: %s\n", EC_POINT_point2hex(group, C1, POINT_CONVERSION_UNCOMPRESSED, ctx));
    ...
    const BIGNUM *tmp = EC_KEY_get0_private_key(key);
    
    printf("** debug DEC PRIVATE KEY : %s\n", BN_bn2hex(tmp));
    //const EC_POINT *Pri = EC_POINT_bn2point(group, tmp, NULL, ctx);
    //printf("** debug ENC PRIVAATE KEY : %s\n", EC_POINT_point2hex(group, Pri, POINT_CONVERSION_UNCOMPRESSED, ctx));

    if (!EC_POINT_mul(group, C1, NULL, C1, EC_KEY_get0_private_key(key), ctx) ) {
        printf("error:CMM_P1_decrypt: DECRYPT, ERR_R_EC_LIB\n");
        goto done;
    }
    printf("** debug: C1 <== C1*privatekey: %s\n", EC_POINT_point2hex(group, C1, POINT_CONVERSION_UNCOMPRESSED, ctx));

    //TEST    
    if (!EC_POINT_mul(group, C1Tmp, NULL, C1Tmp, Pub, ctx) ) {
        printf("error:CMM_P1_decrypt: DECRYPT, C1Tmp ERR_R_EC_LIB\n");
        goto done;
    }
    printf("** debug: C1Tmp <== C1*privatekey: %s\n", EC_POINT_point2hex(group, C1Tmp, POINT_CONVERSION_UNCOMPRESSED, ctx));

    //TEST
    if (!EC_POINT_get_affine_coordinates(group, C1Tmp, x1tmp, y1tmp, ctx)) {
        printf("error:CMM_P1_decrypt: DECRYPT, ERR_R_EC_LIB, C1Tmp --> (x1tmp, y1tmp)\n");
        goto done;
    }
    

    printf("** debug: C1Tmp --> x1tmp by pubkey: %s\n", BN_bn2hex(x1tmp));
    printf("** debug: C1Tmp --> y1tmp by pubkey: %s\n", BN_bn2hex(y1tmp));
    ...
    printf("** debug: C1 --> x2: %s\n", BN_bn2hex(x2));
    printf("** debug: C1 --> y2: %s\n", BN_bn2hex(y2));
    ...
    printf("** debug: DEC BN(x2, y2) ==> BN x2y2:\n");
    for(i=0; i<field_size*2; i++) {
        printf("%d: ", x2y2[i] );
    }
    printf("** debug: ---------- \n");

    printf("** debug DEC msg_len: %d\n", msg_len); 
    printf("** debug DEC msg_mask:\n"); 
    for (i = 0; i != msg_len; ++i) {
        printf("%d: ", msg_mask[i]);
    }
    ...
    printf( "** debug final original text : %s\n", ptext_buf);
...
int CMM_P1_encrypt_file(char *infile, char *certin, char *outfile, int opt_secret) 
{
    ...
    printf("** debug: get public key from certificate...\n");
